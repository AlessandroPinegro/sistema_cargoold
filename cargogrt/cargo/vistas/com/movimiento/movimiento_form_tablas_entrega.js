...
$(document).ready(function () {
    ...
    ax.setAccion("obtenerConfiguracionesInicialesEntrega");
    ax.consumir();
    select2.iniciar();
...
function onResponseMovimientoFormTablas(response) {
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerConfiguracionesInicialesEntrega':
...
function onResponseObtenerConfiguracionesIniciales(data) {
    ...
    if (!isEmpty(data.detalleDocumento)) {
        ...
        if (isEmpty(detalle)) {
            mostrarValidacionLoaderClose("El documento no tiene articulos disponibles para entregar");
...
function guardar(accion) {
    ...
    if (accion == "guardarPago" || accion == "confirmacionPago") {
        ...
        if (accion == "guardarPago" && obtenerCodigoModalidad() != '1' && (dataDocumento['cantidad_desbloqueo'] * 1) == 0) {
            $('#modalNuevoDocumentoPagoConDocumento').modal('hide');
        ...
        //fin documento pago    
    } else if (obtenerCodigoModalidad(dataDocumento['modalidad_id']) == '0'
        && dataDocumento['comprobante_tipo_id'] != 191
        && dataDocumento['bandera_es_cargo'] != 1
        && isEmpty(dataDocumento['documento_tipo_pago'])) {
        //Si modalidad contra entrega y ademas el comprobante no es nota de venta se paga
        abrirModalPagos(dataCofiguracionInicial);
        return;
    } else if (accion != 'confirmacion' && obtenerCodigoModalidad() != '1' && dataDocumento['bandera_es_cargo'] != 1 && (dataDocumento['cantidad_desbloqueo'] * 1) == 0) {
        document.getElementById("btnConfirmacionRegistro").onclick = function (event) {
    ...
    if (accion != 'confirmacionPago' && accion != 'confirmacion') {
        let mensaje = 'Esta a punto de generar la entrega';
        if (accion == "guardarPago") {
...
function onResponseEnviar(data) {
    ...
    if (!isEmpty(data.generarDocumentoAdicional)) {
        ...
        if (!isEmpty(data.dataPlantilla)) {
            ...
            if (!isEmpty(data.url)) {
                abrirDocumentoPDF(data, 'vistas/com/movimiento/documentos/');
            }
