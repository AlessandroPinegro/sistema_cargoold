...
$(document).ready(function () {
    ...
    //    var valorLimpio = valor.replace(/^[^T]|[^\w\d]/g, ''); 
    //   serie.value = valorLimpio.slice(0, 4); 
       
   
      var valor = serie.value.toUpperCase(); 
      if (!/^T/.test(valor)) {
            if(/^[0-9]+$/.test(serie.value)){
                 console.log("llego numeros");
                // var numeros = valor.replace(/\D/g, '');
                // serie.value = numeros;
            }else{
                var valorLimpioEG07 = valor.replace(/[^EG07|EG02]/g, ''); 
                serie.value = valorLimpioEG07;
            }
            
      } else {
          // Reemplazar caracteres no deseados, excepto 'T'
          var valorLimpio = valor.replace(/^[^T]|[^\w\d]/g, '');
          // Asignar el valor limpio a la serie, limitado a los primeros 4 caracteres
          serie.value = valorLimpio.slice(0, 4);
      }
      }if(tipoDoc.value == "31"){
        var valor = serie.value.toUpperCase(); 
            if (!/^V/.test(valor)) {
                if(/^\d+/.test(valor) ){
                    var valorLimpioEG07 = valor.replace(/[^EG03|EG04]/g, ''); 
                    serie.value = valorLimpioEG07;
                }else{
                    var valorLimpioEG07 = valor.replace(/[^EG03|EG04]/g, ''); 
                    serie.value = valorLimpioEG07;
              ...
              // Reemplazar caracteres no deseados, excepto 'T'
              var valorLimpio = valor.replace(/^[^V]|[^\w\d]/g, '');
              // Asignar el valor limpio a la serie, limitado a los primeros 4 caracteres
              serie.value = valorLimpio.slice(0, 4);
          }
        ...
        // serie.value = valorLimpio.slice(0, 4);
        var valor = serie.value.toUpperCase();
        var valorLimpio = valor.replace(/[^BEB03\d]/g, "");     
       if (valorLimpio.length > 0) {
           if (isNaN(valorLimpio.charAt(0)) && valorLimpio.charAt(0) !== 'E' && valorLimpio.charAt(0) !== 'B') {
               valorLimpio = 'E' + valorLimpio.slice(1);
           }
       }
       if (valorLimpio.length > 1 && valorLimpio.charAt(1) === 'E') {
           valorLimpio = valorLimpio.slice(0, 1) + valorLimpio.slice(2);
       }
       serie.value = valorLimpio.slice(0, 4);
        ...
        var valor = serie.value.toUpperCase(); 
       // var valorLimpio = valor.replace(/[T][^W-Z0-9]/g, ''); 
        var valorLimpio = valor.replace(/[^A-Z0-9]/g, '');
        serie.value = valorLimpio.slice(0, 4); 
       
    //     var valor = serie.value.toUpperCase();
    //     var valorLimpio = valor.replace(/[^FE01\d]/g, "");     
    //    if (valorLimpio.length > 0) {
    //        if (isNaN(valorLimpio.charAt(0)) && valorLimpio.charAt(0) !== 'F' && valorLimpio.charAt(0) !== 'E') {
      ...
      if ( nuevoDato1 && nuevoDato2 && nuevoDato3 && nuevoDato4 && textoSeleccionado) {
        if (nuevoDato3 == "31" ) {
          var cantidadObjetosTipo31 = datosArray.filter(function (dato) {
           ...
           console.log(cantidadObjetosTipo31);
            ...
            if(/^[a-zA-Z]+$/.test(serie.value)){
                ...
                if(nuevoDato2 =='20132272418'){
                    datosArray.push({
                     ...
                     mostrarValidacionLoaderClose("Si la serie empieza con una letra El RUC de transportista debe ser igual al RUC de Ittsa");
                  //   numDoc.value = "20132272418";
                    }
            }
                ...
                // Si la entrada contiene una combinación de letras y números
                    ...
                    if( numDoc.value  ==  filtaridcliente[0].codigo_identificacion){
                        datosArray.push({
                         ...
                         mostrarValidacionLoaderClose("La serie empieza con una letra por lo tanto debe ser igual El numero de RUC del remitente");
                         return;
                     }                
                }
        }
        ...
        mostrarValidacionLoaderClose("Registrar todo los campos");
          ...
          if(nuevoDato3 == "31"){
           const  numero = "20132272418";
           numDoc.value="";
            let index = 0;
            const interval = setInterval(() => {
                if (index < numero.length) {
                    numDoc.value += numero[index];
                    index++;
                } else {
                    clearInterval(interval);
                    console.log("Se ingresó el número completo");
                     if(datosArray.length == 0 || datosArray.length == 1){
                        numDoc.value = numero;
                    //  serie.value = "";
                    // correlativo.value = "";
                     }else{
  ...
  //  ::::::::::::::DESARROLLADO POR JESUS:::::::::::::::::
   ...
   //if (!isEmpty(valor) && valor != 6)								
	...
	let valorIdentificador = select2.obtenerValorDataSet("cboComprobanteTipo", "identificador_negocio");
        $("#divGuiaSerieTransportista").show()
        $("#divGuiaCorrelativoTransportista").show()
