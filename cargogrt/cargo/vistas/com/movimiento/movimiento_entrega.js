...
function onResponseObtenerConfiguracionesIniciales(data) {
    ...
    let agenciaDestinoId = data.dataAgencia[0]['id'];
    ...
    if (!isEmpty(data.dataDocumento)) {
        ...
        clienteId = data.dataDocumento[0]['persona_id'];
        ...
        $('#txtFechaEmision').val(datex.parserFecha(data.dataDocumento[0]['fecha_emision'].date));
    }
...
//    cambiarTituloDescripcion('Origen');

    select2.cargar("cboAgenciaDestino", data.dataAgencia, "id", "descripcion_corta");
...
//    cambiarTituloDescripcion('Destino');
    ...
    if (!isEmpty(dataDocumentoTipoDato)) {
        ...
        if (!isEmpty(documentoTipoDato)) {
            ...
            select2.cargar("cboCliente", dataPersona, "id", ["codigo_identificacion", "nombre"]);
            select2.cargar("cboDestinatario", dataPersona, "id", ["codigo_identificacion", "nombre"]);
            select2.cargar("cboAutorizado", dataPersona, "id", ["codigo_identificacion", "nombre"]);
            ...
            if (!isEmpty(destinatarioId)) {
                ...
                banderaCargaInicialDireccion = true;
    ...
    select2.cargar("cboComprobanteTipo", data.documentoTipo, "id", "descripcion");
...
function cargarPersona(tipo)
{
    var rutaAbsoluta = URL_BASE + 'index.php?token=1&tipo=' + tipo;
...
function editarPersona(tipo)
{
    if (!isEmpty(dataPersona)) {
        ...
        if (!isEmpty(personaId)) {
            let persona = dataPersona.filter(item => item.id == personaId);
            var rutaAbsoluta = URL_BASE + 'index.php?token=1&id=' + persona[0].id + '&tipo=' + persona[0].persona_tipo_id + '&retorno=' + tipo;
            window.open(rutaAbsoluta, "nuevo", "directories=no, location=no, menubar=no, scrollbars=yes, statusbar=no, tittlebar=no, width=1500, height=900");
        } else {
...
function obtenerPersonas(tipo) {
    ...
    ax.addParamTmp("documentoTipoId", dataCofiguracionInicial.movimientoTipo[0]['documento_tipo_defecto_id']);
    ax.setTag(tipo);
    ax.consumir();
}
...
//    return;
    ...
    if (!isEmpty(importes.calculoId)) {
        ...
        calculo = calculo + costo_reparto + importe_devolucion_cargo + importe_otro_cargo;
