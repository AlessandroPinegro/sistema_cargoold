...
//FUNCIONES PARA COPIAR DOCUMENTO
            ...
            case 'obtenerProductos':
                ...
                loaderClose();
                break;
            case 'obtenerAtencion':
                onResponseObtenerAtencion(response.data);
                loaderClose();
                break;
...
function obtenerStockActual(indice) {
    ...
    if (!isEmpty(bienId)) {
        ...
        ax.addParamTmp("documentoRelacion", request.documentoRelacion);
        ax.setTag(indice);
        ax.consumir();
    }
}
...
//                    }
            ...
            switch (parseInt(item.tipo)) {
                ...
                case 7:
                    ...
                    $("#contenedorSerie").html('<input type="text" id="txt_' + item.id + '" name="txt_' + item.id + '" class="form-control" value="' + value + '" maxlength="' + longitudMaxima + '" placeholder="Serie" readonly style="text-align: right;"/>');
                    break;
...
function cargarModalCopiarDocumentos() {
    ...
    setTimeout(function(){ cambiarAnchoBusquedaDesplegable(); }, 1000);
}
...
function getDataTableDocumentoACopiar()
{
    ...
    $('#dtDocumentoRelacion').dataTable({
        ...
        "order": [[0, "desc"]],
        "columns": [
            {"data": "fecha_programada", "width": "15%"},
            {"data": "fecha_emision", "width": "10%"},
            {"data": "persona", "width": "30%"},
            {"data": "serie_numero", "width": "15%"},
            {"data": "serie_numero_original", "width": "15%"},
        ...
        columnDefs: [
            {
                "render": function (data, type, row) {
                    return (isEmpty(data)) ? '' : row.fecha_emision_format;
                },
                "targets": 1
            }, {
                "render": function (data, type, row) {
                    return (isEmpty(data)) ? '' : row.fecha_programada_format;
                },
                "targets": 0
            }
        ],
...
function agregarDocumentoRelacion(documentoTipoOrigenId, documentoId, movimientoId, monedaId, relacionar)
{
    ...
    if (obtenerNumeroDeDocumentosCopiados() >= 1) {
        mostrarAdvertencia("Solo se puede relacionar un documento");
        loaderClose();
        return;
    }
...
function obtenerNumeroDeDocumentosCopiados() {
    var num = 0;
...
function cargarDataTableDocumentoACopiar(data)
{
    if (!isEmpty(data))
    {
        ...
        if (!isEmpty(detalleLink))
        {
            ...
            $('#linkDocumentoACopiar').append("<a id='link_" + contadorDocumentoCopiadoAVisualizar + "' onclick='visualizarDocumentoRelacion(" + contadorDocumentoCopiadoAVisualizar + ")' id='linkDocumentoACopiar' style='color:#0000FF'>[" + detalleLink + "]</a>");
            $('#linkDocumentoACopiar').append("<a id='cerrarLink_" + contadorDocumentoCopiadoAVisualizar + "' onclick='eliminarDocumentoACopiar(" + contadorDocumentoCopiadoAVisualizar + ")'style='color:red;'>&ensp;X</a>" +
                    "<a id='obtenerAtenciones_" + contadorDocumentoCopiadoAVisualizar + "' title='Ver programación de atenciones' onclick='obtenerAtencion(" + contadorDocumentoCopiadoAVisualizar + ")'>&ensp;<i class='fa fa-eye' style='color:b'></i></a><br>");
            $("#divDocumentoRelacion").css("height", $("#divDocumentoRelacion").height() + 20);
...
function obtenerAtencion(indice) {
    var documentoId = request.documentoRelacion[indice].documentoId;
    
    loaderShow();
    ax.setAccion("obtenerAtencion");
...
function onResponseObtenerAtencion(data) {
    dataInicial = data;
...
function obtenerIconoEstado(indice, estadoId) {
    ...
    switch (estadoId * 1) {
        ...
        case 3:
            ...
            break;
        case 4:
            html = html + "<i class='fa fa-lock' style='color:red;' title='Comprometido'></i>&nbsp;&nbsp;&nbsp;";
            break;
        case 5:
            html = html + "<i class='fa fa-th-large' style='color:orange;' title='Atendido parcialmente'>&nbsp;&nbsp;&nbsp;";
            break;
        case 6:
...
//                    + "<td style='text-align:center;'>" + iconoEstado + "</td>"
            if (item.estadoId == 5) {
                html += "<td style='text-align:center;'>Atendido " + formatearCantidad(item.cantidadAtendida) + "</td>"
            } else {
