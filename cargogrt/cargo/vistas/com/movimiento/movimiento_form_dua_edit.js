...
$(document).ready(function () {
    ...
    var movimientoTipoId = document.getElementById("movimientoTipoId").value;

    ax.setSuccess("onResponseMovimientoFormDua");
    ax.addParamTmp("empresaId", commonVars.empresa);
    ax.addParamTmp("documentoId", documentoId);
    ax.addParamTmp("movimientoTipoId",movimientoTipoId);    
...
function onResponseMovimientoFormDua(response) {
...
//        obtenerStockActual(data.indice);
        ...
        if (existeColumnaCodigo(20)) {
            document.getElementById("txtAdValorem_" + data.indice).value = "0.00";
        }
        hallarSubTotalDetalle(data.indice);
    }

    //HABILITAR O DESHABILITAR EL AD VALOREM
    var bienTipoId = obtenerBienTipoIdPorBienId(select2.obtenerValor("cboBien_" + data.indice));
    if (bienTipoId == -4) {
        $('#txtAdValorem_' + data.indice).attr('disabled', 'disabled');
    } else {
        $('#txtAdValorem_' + data.indice).removeAttr("disabled");
    }
    //FIN HABILITAR O DESHABILITAR EL AD VALOREM
...
//        }
        ...
        if (!isEmpty(data.dataDocumento)) {
            ...
            agregarDocumentoRelacion(dataDoc.documento_tipo_id, dataDoc.id, dataDoc.movimiento_id, dataDoc.moneda_id, 1);
            select2.readonly("cboDocumentoTipo", true);
        }
        ...
        if (!isEmpty(dataCofiguracionInicial.contOperacionTipo)) {
            ...
            select2.cargar("cboOperacionTipo", dataContOperacionTipo, "id", "descripcion");
            select2.asignarValor("cboOperacionTipo", dataCofiguracionInicial.dataDocumento[0].cont_operacion_tipo_id);
            select2.readonly("cboOperacionTipo", true);

//            if (dataContOperacionTipo.length === 1) {
//                select2.readonly("cboOperacionTipo", true);
//            }
        }
    }
...
//    $('#contenedorPercepcionDiv').append("<median class='text-uppercase' id='simPercepcion'>" + monedaSimbolo + "</median>");
    ...
    $('#contenedorCifDiv').append("<median class='text-uppercase' id='simSubTotal'>" + monedaSimbolo + "</median>");
    $('#contenedorFobDiv').append("<median class='text-uppercase' id='simSubTotal'>" + monedaSimbolo + "</median>");
    $('#contenedorFleteSunatDiv').append("<median class='text-uppercase' id='simSubTotal'>" + monedaSimbolo + "</median>");
    $('#totalUtilidadDescripcion').append(" " + "<median class='text-uppercase' id='simTotalUtildiad'>" + monedaSimbolo + "</median>");
    $('#contenedorSeguroDiv').append("<median class='text-uppercase' id='simSubTotal'>" + monedaSimbolo + "</median>");
    $('#contenedorFleteDiv').append("<median class='text-uppercase' id='simSubTotal'>" + monedaSimbolo + "</median>");
...
//            breakFunction();
            switch (parseInt(item.codigo)) {
                ...
                case 18://fecha de vencimiento
                    ...
                    anchoDinamicoTabla += parseInt(item.ancho);
                    break;
                case 19://Afecto / Inafecto
                    fila += "<th style='text-align:center; width: " + item.ancho + "px;'>" + item.descripcion + "</th>";
                    anchoDinamicoTabla += parseInt(item.ancho);
                    break;
                case 20://Ad valorem
                    fila += "<th style='text-align:center; width: " + item.ancho + "px;'>" + item.descripcion + "<div id='simAV' style='display: inline-table;'>" + monedaSimbolo + "</th>";
                    anchoDinamicoTabla += parseInt(item.ancho);
                    break;
            }
        });
        ...
        $.each(dataColumna, function (index, item) {
            switch (parseInt(item.codigo)) {
                ...
                case 18://Fecha de vencimiento
                    ...
                    break;
                case 19://Fecha de vencimiento
                    fila += "<td style='border:0; width: " + item.ancho + "px; vertical-align: middle; padding: " + KPADINGTD + "px;'>" + agregarAfectoInafectoDetalleTabla(indice) + "</td>";
                    break;
                case 20://Ad Valorem
                    fila += "<td style='border:0; width: " + item.ancho + "px; vertical-align: middle; padding: " + KPADINGTD + "px;'>" + agregarAdValoremDetalleTabla(indice) + "</td>";
                    break;
            }
        });
    }
...
function agregarAfectoInafectoDetalleTabla(i) {
    var $html = "<label class='cr-styled' style='text-align: left;'>" +
            "<input type='checkbox' id='chkAfecto_" + i + "' checked='true'>" +
            "    <i class='fa'></i></label>";
    return $html;
}
...
function agregarAdValoremDetalleTabla(i) {
    var $html = "<div class=\"input-group col-lg-12 col-md-12 col-sm-12 col-xs-12\">" +
            "<input type=\"number\" id=\"txtAdValorem_" + i + "\" name=\"txtAdValorem_" + i + "\" class=\"form-control\" required=\"\" aria-required=\"true\" value=\"\" style=\"text-align: right;\" onchange=\"hallarSubTotalDetalle(" + i + ")\" onkeyup =\"hallarSubTotalDetalle(" + i + ")\"/></div>";
...
function actualizarTotalesGenerales(indice) {
    ...
    var subTotal = valoresFormularioDetalle.cantidad * valoresFormularioDetalle.precio + valoresFormularioDetalle.adValorem * 1;
...
//                    valor = document.getElementById("txtPrecioCompra_" + indice).value;
                ...
                case 18://fecha vencimiento
                    ...
                    break;
                case 20:// Ad Valorem
                    valor = document.getElementById("txtAdValorem_" + indice).value;
                    objDetalle.adValorem = valor;
                    break;
            }
...
//                    importes.noAfectoId = 'txt_' + item.id;
                    importes.noAfectoId = 'txt_percepcion';
                    $("#contenedorPercepcionDiv").show();
//chl                    $("#contenedorPercepcion").html('<input type="number" id="txt_' + item.id + '" name="txt_' + item.id + '" ' + readonly + ' class="form-control" value="" maxlength="' + longitudMaxima + '"  ' + maxNumero + ' style="text-align: center;" onchange="calculeTotalMasPercepcion(' + item.id + ')"  disabled/>');
                    break;
                case 2:
                case 6:
                case 12:
...
//                    $("#contenedorCambioPersonalizado").show();
                    $("#cambioPersonalizado").html('<input type="number" id="txt_' + item.id + '" name="txt_' + item.id + '" ' + readonly +
                            ' onkeyup="if(this.value.length>25){this.value=this.value.substring(0,25)} onChangeTipoCambioPersonalizaddo();" ' +
                            ' onchange="onChangeTipoCambioPersonalizaddo()" ' +
                            ' class="form-control" value="" maxlength="' + longitudMaxima + '"  ' + maxNumero + ' style="text-align: right;" placeholder="' + item.descripcion + '"/>');
                    break;
                case 25:
                    $("#divContenedorTipoPago").show();
                    break;
                ...
                case 27:
                    ...
                    break;
                case 28:
                    importes.cifId = 'txt_' + item.id;
                    $("#contenedorCifDiv").show();
                    $("#contenedorCif").html('<input type="number" id="' + importes.cifId + '" name="' + importes.cifId + '" ' + readonly + ' class="form-control" value="" maxlength="' + longitudMaxima + '"  ' + maxNumero + ' style="text-align: center;" />');
                    break;
                case 29:
                    importes.fobId = 'txt_' + item.id;
                    $("#contenedorFobDiv").show();
                    $("#contenedorFob").html('<input type="number" id="' + importes.fobId + '" name="' + importes.fobId + '" ' + readonly + ' class="form-control" value="" maxlength="' + longitudMaxima + '"  ' + maxNumero + ' style="text-align: center;" />');
                    break;
            }
            if (escribirItem) {
                html += '</div></div>';
...
//                case 9:
                ...
                case 5:
                    ...
                    select2.asignarValor("cbo_" + item.id, persona_sunat_id);
                    break;
                case 17:
                    $("#cbo_" + item.id).select2({
                        width: '100%'
        ...
        $.each(dataColumna, function (index, item) {
            ...
            switch (parseInt(item.codigo)) {
                ...
                case 6:// SUB TOTAL
                    ...
                    break;
                case 20:// AD VALOREM
                    $('#txtAdValorem_' + indexDetalle).val(devolverDosDecimales(valoresFormularioDetalle.adValorem));
                    break;
                case 9:// PRIORIDAD
                    if (dataCofiguracionInicial.movimientoTipo[0].interfaz == 1) {
...
function cargarPantallaListar() {
     if (isEmpty(opcionAnterior)) {
        cargarDiv("#window", "vistas/com/movimiento/movimiento_listar_dua.php?tipoInterfaz=" + tipoInterfaz);
    } else {
        cargarDiv("#window", "vistas/com/contabilidadRevisionDocumentos/documento_revision.php");
    }
}
...
function enviar(accion) {
    ...
    // Validamos si la percepción está habilitada
    if (percepcion.estadoEdicion == 1) {
        mostrarAdvertencia("Debe culminar con el registro de percepción.");
        return false;
    }
    ...
    $.each(camposDinamicos, function (index, item) {
        ...
        switch (item.tipo) {
            ...
            case 33:
                ...
                break;
            case 19:
                var numero = document.getElementById("txt_percepcion").value;
                if (isEmpty(numero)) {
                    if (item.opcional == 0) {
                        mostrarValidacionLoaderClose("Debe ingresar " + item.descripcion);
                        isOk = false;
                ...
                break;
//            case 30:
//                var numero = (document.getElementById('chkFleteSunat').checked) ? document.getElementById("txt_FleteSunat").value : 0;
//                if (isEmpty(numero)) {
//                    mostrarValidacionLoaderClose("Debe ingresar " + item.descripcion);
//                    isOk = false;
//                    return false;
...
//        return;
    ...
    ax.addParamTmp("percepcion",
            {
                importeSoles: percepcion.importeSoles,
                importeDolares: percepcion.importeDolares
            });
...
function calcularImporteDetalle() {
    var importesCalc = {
        ...
        cif: 0,
        fob: 0,
        noAfecto: 0, // importeSinIGV
        afecto: 0, // importeConIGV
        temporal: 0
    };

    importesCalc.noAfecto = obtenerPercepcionMonedaActual();

    if (movimientoTipoIndicador != 5) {
        if (!isEmpty(detalle)) {
//            breakFunction();
            $.each(detalle, function (index, item) {
                importesCalc.temporal = (parseFloat(item.cantidad) * parseFloat(item.precio));
//                importesCalc.afecto = importesCalc.temporal+item.adValorem*1;
                switch (item.bienId * 1) {
//                    case bienIds.adValorem:
//                        importesCalc.afecto += importesCalc.temporal;
//                        break;
                    case bienIds.flete:
                    case bienIds.fleteAereo:
//                        breakFunction();
//                        if (document.getElementById('chkFleteSunat').checked){
//                            importesCalc.afecto += parseFloat(document.getElementById("txt_FleteSunat").value);
//                        }else{
//                            importesCalc.afecto += importesCalc.temporal;
//                        }
//                        importesCalc.cif += importesCalc.temporal;
                        break;
                    case bienIds.seguro:
                    case bienIds.comision:
                    case bienIds.otrosCostos:
//                        importesCalc.afecto += importesCalc.temporal;
//                        importesCalc.cif += importesCalc.temporal;
                        break;
//                    case bienIds.htc:
//                        importesCalc.afecto += importesCalc.temporal;
//                        importesCalc.cif += importesCalc.temporal;
//                        importesCalc.fob += importesCalc.temporal;
                        break;
                    default:
                        importesCalc.cif += importesCalc.temporal;
                        importesCalc.fob += importesCalc.temporal;
                        importesCalc.afecto += importesCalc.temporal;
                        importesCalc.addValorem += (isEmpty(item.adValorem) ? 0 : (item.adValorem * 1));
                        break;
                }
...
function asignarImporteDocumento() {
    ...
    if (!isEmpty(importes.calculoId)) {
        ...
        if (!isEmpty(importes.fleteId) && !isEmpty(importes.seguroId)) {
            ...
            calculo.afecto = calculo.afecto + importe_seguro + importe_flete;
            calculo.cif = calculo.cif + importe_seguro + importe_flete;
        }

        document.getElementById(importes.subTotalId).value = devolverDosDecimales(calculo.afecto + calculo.addValorem);
        igv = Math.round(igvValor * (calculo.afecto + calculo.addValorem));
        document.getElementById(importes.igvId).value = devolverDosDecimales(igv);
//        document.getElementById(importes.noAfectoId).value = devolverDosDecimales(calculo.noAfecto);
        document.getElementById(importes.totalId).value = devolverDosDecimales(calculo.afecto + igv + calculo.noAfecto + calculo.addValorem);
        document.getElementById(importes.cifId).value = devolverDosDecimales(calculo.cif);
        document.getElementById(importes.fobId).value = devolverDosDecimales(calculo.fob);
...
//            }
//        }
        calculoTotal = parseFloat(document.getElementById(importes.totalId).value);
    }
...
//    breakFunction();
    ...
    if (!isEmpty(importes.calculoId)) {
        ...
        if (percepcionId != 0) {
            valorPercepcion = obtenerPercepcionMonedaActual();
        ...
        $.each(data, function (index, item) {
            cargarDataTableDocumentoACopiar(
                    cargarFormularioDetalleACopiar(item.organizador_id, item.bien_id, item.cantidad,
                            ...
                            item.movimiento_bien_detalle, item.dataUnidadMedida,item.ad_valorem,item.movimiento_bien_id));
        });
...
function cargarFormularioDetalleACopiar(organizadorId, bienId, cantidad, unidadMedidaId, precio,
        organizadorDesc, bienDesc, unidadMedidaDesc, precioTipoId, movimientoBienDetalle, dataUnidadMedida,addValorem,movimientoBienId) {
//    debugger;
        ...
        $.each(dataColumna, function (index, item) {
            ...
            switch (parseInt(item.codigo)) {
                ...
                case 15:// Organizador
                    ...
                    break;
                case 20:// ADVALOREM  
                    objDetalle.adValorem = addValorem;
                    break;
...
//        $('#dtBodyDocumentoGenerado').append(cuerpo);
        ...
        } else if (!isEmpty(data.documentoEar)) {
            //MODAL DE EARs 
            onResponseObtenerDocumentosEarDua(data);
...
//    console.log(e);
    if (e.target.id != "btnBusqueda" && e.delegateTarget.id != "ulBuscadorDesplegable2" && e.delegateTarget.id != "listaEmpresa" && e.delegateTarget.id != "mnuPercepcion") {
        e.stopPropagation();
    }
});
...
function obtenerValoresCamposDinamicosPago() {
    ...
    $.each(camposDinamicos, function (indexCampos, itemCd) {
        $.each(dataCofiguracionInicial.documento_tipo_conf, function (indexConfig, itemDtd) {
...
function onChangeCheckFleteSunat() {
    if (document.getElementById('chkFleteSunat').checked) {
        $("#txt_FleteSunat").removeAttr("readonly");
    } else {
        $("#txt_FleteSunat").attr('readonly', 'true');
    }
    document.getElementById("txt_FleteSunat").value = "0.00";
    asignarImporteDocumento();
}
...
//    } else {
//        $('#txt_' + percepcionId).val('0.00');
//        $('#' + importes.totalId).val(devolverDosDecimales(calculoTotal));
//        $('#txt_' + percepcionId).attr('disabled', 'disabled');
//    }
//}

//CHL: Funcinalidad percepcion 
function cambiaPercepcionASoles() {
    $("#lblMonedaPercepcion").html("S/.");
    document.getElementById("txt_percepcion").value = percepcion.importeSoles;
    percepcion.monedaActualId = 2;
}
function cambiaPercepcionADolares() {
    $("#lblMonedaPercepcion").html("$.");
    document.getElementById("txt_percepcion").value = devolverDosDecimales(percepcion.importeDolares);
    percepcion.monedaActualId = 4;
}
function habilitarEdicionPercepcion() {
    $("#txt_percepcion").removeAttr("readonly");
    $("#btnCambiaMonedaPercepcion").attr("disabled", "true");
    $("#divHabilitaEdicionPercepcion").hide();
    $("#divAplicaPercepcion").show();
    cambiaPercepcionASoles();
    percepcion.estadoEdicion = 1;
}
function aplicarEdicionPercepcion() {
    percepcion.importeSoles = (isEmpty(document.getElementById("txt_percepcion").value)) ? 0 : document.getElementById("txt_percepcion").value;
    calculaPercepcioDolares();
    if (select2.obtenerValor("cboMoneda") == 4) {
        cambiaPercepcionADolares();
    }

    asignarImporteDocumento();

    $("#txt_percepcion").attr("readonly", "true");
    $("#btnCambiaMonedaPercepcion").removeAttr("disabled");
    $("#divHabilitaEdicionPercepcion").show();
    $("#divAplicaPercepcion").hide();
    percepcion.estadoEdicion = 0;
}
function calculaPercepcioDolares() {
    var tc = (isEmpty($("#txt_" + cambioPersonalizadoId).val())) ? 0 : $("#txt_" + cambioPersonalizadoId).val();
    percepcion.importeDolares = percepcion.importeSoles / tc;
}
function onChangeTipoCambioPersonalizaddo() {
    calculaPercepcioDolares();
    if (percepcion.monedaActualId == 4) {
//        breakFunction();
        cambiaPercepcionADolares();
        asignarImporteDocumento();
    }
}
function iniciaPercepcion() {
    percepcionId = 0;
    percepcion.importeSoles = 0;
    percepcion.importeDolares = 0;
    percepcion.monedaActualId = 2;
    cambiaPercepcionASoles();
    $("#divAplicaPercepcion").hide();
}
function obtenerPercepcionMonedaActual() {
    return ((select2.obtenerValor("cboMoneda") == 2) ? percepcion.importeSoles : percepcion.importeDolares);
}
...
var documentoDuaId;
function onResponseObtenerDocumentosEarDua(data) {
    var dataDocumentosEar = data.documentoEar;
    var dataDocumentoDua = data.documentoDua[0];

    if (!isEmptyData(dataDocumentosEar)) {
        documentoDuaId = data.documentoId;
        //TITULO PARA MODAL
        var tituloModal = 'EXISTE EAR DISPONIBLE REGISTRADA CON S/N DUA (' + dataDocumentoDua.serie_dua + '-' + dataDocumentoDua.numero_dua + ')';
