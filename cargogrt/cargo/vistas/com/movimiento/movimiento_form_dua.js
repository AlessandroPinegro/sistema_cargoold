...
//        }
        ...
        if (!isEmpty(dataCofiguracionInicial.contOperacionTipo)) {
            ...
            select2.asignarValor("cboOperacionTipo", dataContOperacionTipo[0].id);
...
function cargarPantallaListar() {
    cargarDiv("#window", "vistas/com/movimiento/movimiento_listar_dua.php?tipoInterfaz=2");
}
    ...
    if (!bandera.mostrarDivDocumentoRelacion)
    {
        if (!isEmpty(data))
        {
            ...
            $.each(data, function (index, item) {
                ...
                switch (parseInt(item.tipo)) {
                    case 5:
                        select2.asignarValor('cbo_' + item.otro_documento_id, persona_sunat_id);
        ...
        $.each(data, function (index, item) {
            ...
            cargarDataTableDocumentoACopiar(
                    cargarFormularioDetalleACopiar(item.organizador_id, item.bien_id, item.cantidad,
                            ...
                            item.movimiento_bien_detalle, item.dataUnidadMedida
                            )
                    );
...
function cargarFormularioDetalleACopiar(organizadorId, bienId, cantidad, unidadMedidaId, precio,
        organizadorDesc, bienDesc, unidadMedidaDesc, precioTipoId, movimientoBienDetalle, dataUnidadMedida) {
        ...
        $.each(dataColumna, function (index, item) {
            ...
            switch (parseInt(item.codigo)) {
                ...
                case 15:// Organizador
                    ...
                    break;
                case 20:// ADVALOREM (POR EL MOMENTO NO TRAE DATA)
                    objDetalle.adValorem = 0;
                    break;
