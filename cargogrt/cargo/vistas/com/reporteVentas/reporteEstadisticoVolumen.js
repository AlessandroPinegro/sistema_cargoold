...
function onResponseObtenerConfiguracionesIniciales(data) {
    ...
    select2.asignarValor("cboTipoFrecuencia",2);
    ...
    if(!isEmpty(data.dataMoneda)){
        select2.cargar("cboMoneda", data.dataMoneda, "id", ["descripcion","simbolo"]);       
        select2.asignarValor("cboMoneda",4); 
    }
    loaderClose();
}
...
function cargarDatosBusqueda()
{
    ...
    valoresBusquedaReporteEstadisticoVentas[0].monedaId = tipoMoneda;
}
...
function obtenerDatosBusqueda()
{
    ...
    if (!isEmpty(valoresBusquedaReporteEstadisticoVentas[0].monedaId))
    {
        cadena += negrita("Moneda: ");
        cadena += select2.obtenerText('cboMoneda');
        cadena += "<br>";
    }    
...
function verGraficoEjemplo(data) {   
    ...
    createLineChart('graficoReporteEstadisticoVentas', $data, 'y', ['a', 'b','c'], ['Series A', 'Series B', 'Series C'], ['#1a2942', '#3bc0c3', '#dcdcdc']);
...
function verGrafico(data) {
    ...
    var serieA='Total de ventas';
    var totalVenta=0;
    if (!isEmptyData(data))
    {
        var dataGrafico = [{y: null, a: null}];
        $.each(data, function (index, item) {
            totalVenta=parseFloat(item['total_ventas']).toFixed(2);
            dataGrafico[index] = {y: item['fecha_tiempo'], a: totalVenta};
        });
