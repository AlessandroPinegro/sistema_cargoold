...
require_once __DIR__ . '/PerfilAgenciaCajaNegocio.php';
...
//Cód.	Tipo actividad	Actividad	Total
    ...
    public function obtenerConfiguracionesInicialesCierreCaja($usuarioId) {
        $respuesta = new ObjectUtil();
        //Ingreso: 7,2,3 -- Salida: 8,5,6
...
//        $respuesta->documento_tipo = DocumentoTipoNegocio::create()->obtenerDocumentoTipoXDocumentoTipoDatoXTipo($tipoDato);
        ...
        $respuesta->cuenta = CuentaNegocio::create()->obtenerCuentasActivas();
        //::JESUS
        $respuesta->usuario =    ACCajaNegocio::create()->obtenerUsuarioDatosCaja($usuarioId, 2);
        ...
        $respuesta->dataCajaUsuario = ($banderaTodasCajas ? PerfilAgenciaCajaNegocio::create()->obtenerCajaXAgenciaId($agenciasId) : PerfilAgenciaCajaNegocio::create()->obtenerPerfilAgenciaCajaXUsuarioId($usuarioId));
    ...
    public function reporteCierreCaja($criterios) {
        ...
        $actividadIdFormateado = implode(',',(array) $actividad); // Util::convertirArrayXCadena($actividad);
        $actividadTipoIdFormateado = implode(',',(array) $actividadTipo); // Util::convertirArrayXCadena($actividadTipo);
        $cuentaIdFormateado = implode(',',(array) $cuenta); // Util::convertirArrayXCadena($cuenta);

        $empresaId = $criterios[0]['empresaId'];
        $cajaId = $criterios[0]['cajaId'];
//        $empresaId = $criterios[0]['empresaId'];
        return Reporte::create()->reporteCierreCaja($actividadIdFormateado, $actividadTipoIdFormateado, $fechaEmision, $cuentaIdFormateado, $empresaId, $cajaId);
    }
    ...
    public function reporteCierreCajaTotales($criterios) {
        ...
        $empresaId = $criterios[0]['empresaId'];
        $cajaId = $criterios[0]['cajaId'];
        return Reporte::create()->reporteCierreCajaTotales($actividadIdFormateado, $actividadTipoIdFormateado, $fechaEmision, $cuentaIdFormateado, $empresaId,$cajaId);
    }
