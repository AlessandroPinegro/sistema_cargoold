    ...
    <body style="background-color: white;color: black;">
        ...
        <script>
            ...
            function obtenerParametrosGenerales() {
                $.post('vistas/com/mail/servicio_confirmacion_usuario_activar.php', {
                    accion: 'acivarUsuario',
                    lista: <?php echo "'" . $parametros . "'"; ?>
                }, function (respuesta) {
                    let respuestaActivacion = JSON.parse(respuesta);
                    if (!isEmpty(respuestaActivacion)) {
                        if (respuestaActivacion[0]['vout_exito'] == 1) {
                            mensajeSuccess(respuestaActivacion[0]['vout_mensaje']);
                        } else {
                            mensajeWarning(respuestaActivacion[0]['vout_mensaje']);
                        }
                    } else {
                        mensajeWarning('No se obtuvo la información necesaria para activar el usuario.')
                    }
                });
            }
        </script> 
