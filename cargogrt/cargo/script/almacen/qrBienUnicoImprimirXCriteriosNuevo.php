...
# Espacio de margen superior
?>
    <div style="float: left;width: 49%;; height: 25px;"></div>
    <div style="float: left;width: 49%;; height: 25px;"></div>
    <?php
//IMPRIMIR EN BLANCO
$ind = 0;
...
//    $rutaBienUnico = Configuraciones::url_base() . '?token=3&id=' . $bienUnicoId . '&codigo=' . $bienUnicoCodigo;    
    ...
    $estiloDiv = 'float: left;width: 49%;';
    $anchoTabla = '280px';
    $espacioIzq = '<td  style="height: 68px; padding: 0px; width: 50px"></td>';
    if (($index + $ind + 1) % 2 == 0) {
        $estiloDiv = 'float: right;width: 49%;';
        $anchoTabla = '280px';
        $espacioIzq = '';
    ...
    if (($index + $ind + 1) % 22 == 0) {
        ?>
        <div style="float: left;width: 49%;; height: 80px;"></div>
        <div style="float: right;width: 49%;; height: 80px;"></div>
        <?php
    }
