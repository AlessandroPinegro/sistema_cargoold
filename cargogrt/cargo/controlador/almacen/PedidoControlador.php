...
class PedidoControlador extends MovimientoControlador {
    ...
    public function obtenerConfiguracionesInicialesPedido() {
        ...
        $data = new stdClass();
        $data->persona_activa = PersonaNegocio::create()->obtenerPersonaActivoXStringBusqueda(NULL);
        ...
        $data -> dataObservacion = ObservacionNegocio::create()->obtenerEstadoObservacion();
    ...
    public function obtenerConfiguracionCostoReparto(){
        $idCliente = $this->getParametro("cboCliente");
        return PedidoNegocio::create()->obtenerConfiguracionCostoReparto($idCliente);
    }
    ...
    public function obtenerConfiguracionesInicialesEntrega() {
        ...
        public function obtenerConfiguracionesInicialesPago() {
        $opcionId = $this->getOpcionId();
        $empresaId = $this->getParametro("empresaId");
        $documentoId = $this->getParametro("documentoId");
        $usuarioId = $this->getUsuarioId();
        $movimientoTipoId = $this->getParametro("movimientoTipoId");
        return PedidoNegocio::create()->obtenerConfiguracionesInicialesPago($opcionId, $empresaId, $usuarioId, $documentoId, $movimientoTipoId);
    }
    ...
    public function validarRegistrarEntregar() {
        ...
        $empresaId = $this->getParametro("empresaId");
         ...
         if(!ObjectUtil::isEmpty($agencia)){
             $agencias = implode(",", $agencia);
         }  
        return PedidoNegocio::create()->validarRegistrarEntregar($empresaId, $documentoId, $usuarioId,$agencias);
    }
    
        public function validarRegistrarPago() {
        $usuarioId = $this->getUsuarioId();
        $documentoId = $this->getParametro("documentoId");
        $empresaId = $this->getParametro("empresaId");
        return PedidoNegocio::create()->validarRegistrarPago($empresaId, $documentoId, $usuarioId);
    }

//    public function validarClaveEntregar() {
//        $documentoId = $this->getParametro("documentoId");
//        $clave = $this->getParametro("clave");
//        return PedidoNegocio::create()->validarClaveEntregar($documentoId, $clave);
//    }
    ...
    public function enviar() {
        ...
        $dataguia = $this->getParametro("dataguia");
        if ($documento['documentoTipoId'] == 280) {
            PedidoNegocio::create()->validarClaveSecretaEntrega($opcionId, $documento['documentoOrigenId'], $documento['clave'], $usuarioId);
        }

        $this->setTransaction();
       // return PedidoNegocio::create()->guardar($opcionId, $usuarioId, $documento, $detalle, $dataPago);
       //::DESARROLLADO JESUS
       return PedidoNegocio::create()->guardar($opcionId, $usuarioId, $documento, $detalle, $dataPago,false, $dataguia);
       //::DESARROLLADO JESUS
      ...
      public function enviarPago() {
        ...
        $this->setTransaction();
        return PedidoNegocio::create()->guardarPagoContraEntrega($opcionId, $usuarioId, $documento, $detalle, $dataPago);
    }

    public function enviarPagoC() {
    ...
    public function obtenerDocumentos() {
        ...
        $data = PedidoNegocio::create()->obtenerDocumentosPedidosXCriterios($opcionId, $criterios, $elemntosFiltrados, $columns, $order, $start);
        $elementosTotales = (count($data) > 0 ? $data[0]["total_filas"] : 0);

        $dataAperturaCaja = ACCajaNegocio::create()->obteneACCajaUltimoXUsuarioId($empresaId, $usuarioId, null);
        $cajaDefault = NULL;
        if (!ObjectUtil::isEmpty($dataAperturaCaja)) {
            $cajaDefault = $dataAperturaCaja[0]['caja_id'];
        }
        $cajaRegistro = $data[0]['caja_id'];
        $perfilUsuario = PerfilNegocio::create()->obtenerPerfilXUsuarioId($usuarioId);

        $perfilesFiltrados = array_filter($perfilUsuario, function($perfil) {
            // Aquí especificamos el valor que queremos filtrar
            return $perfil['nombre'] === 'ADMINISTRADOR';
        });
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            //::DESARROLLO JESUS::
            $documentoIdObservacion = $data[$i]['documento_id'];
            $stringAcciones2 = '';
            $stringAcciones2 .= '<a  onclick="ModalObservacion ('.$documentoIdObservacion.')" title="Cambiar estado" id='.$documentoIdObservacion.'><b><i class="fa fa-pencil" style="color:#673ab7"></i></b></a>&nbsp;&nbsp;&nbsp;';
              //::DESARROLLO JESUS::
            for ($j = 0; $j < count($responseAcciones); $j++) {
                if ($estadoId == 2 && $responseAcciones[$j]['id'] != 5) { //ANULAR
                   
                    $stringAcciones .= '' ;
                } elseif (!in_array((int) $estadoId, array(4, 16, 17)) && $responseAcciones[$j]['id'] == 19) { //EDICION
                    $stringAcciones .= '';
                } elseif (($estadoId != 10 && $estadoId != 11 && $estadoId != 15) && $responseAcciones[$j]['id'] == 25) { //Registrar entrega
                    $stringAcciones .= '';
                } elseif ($responseAcciones[$j]['id'] == 24 && ObjectUtil::isEmpty($data[$i]['comprobante_id'])) { //IMPRIMIR COMPROBANTE
                    $stringAcciones .= '';
                } elseif ($responseAcciones[$j]['id'] == 30 && ObjectUtil::isEmpty($data[$i]['guia_transportista_id'])) { //IMPRIMIR COMPROBANTE
                    $stringAcciones .= '';
                } elseif ($responseAcciones[$j]['id'] == 27 && ((int) $data[$i]['pago_contador'] == 0)) { //MOSTRAR EL PAGO
                    $stringAcciones .= '';
                } elseif ($responseAcciones[$j]['id'] == 3 && $cajaDefault == null && $perfilUsuario[0]['id'] != 118) { //MOSTRAR OPCIÓN DE ANULAR
                    $stringAcciones .= '';
                } elseif ($responseAcciones[$j]['id'] == 3 && $cajaDefault != $cajaRegistro && $perfilUsuario[0]['id'] != 118) { //MOSTRAR OPCIÓN DE ANULAR
                    $stringAcciones .= '';
                } else {
                    ...
                    if ($responseAcciones[$j]['id'] == 30) {
                        $documentoId = $data[$i]['guia_transportista_id'];
                    }
            ...
            if ($banderaVisualizar == 1) {
                $stringVisualizarDocumento = '<a  onclick="visualizarDocumento(' . $data[$i]['documento_id'] . ',' . $data[$i]['movimiento_id'] . ')" title="Visualizar" style="cursor:pointer;color:-webkit-link;    text-decoration: underline;">' . $data[$i]['serie_numero'] . '</a>';
            } else {
                $stringVisualizarDocumento = $data[$i]['serie_numero'];
            }

            if ($banderaVisualizar == 1 && !ObjectUtil::isEmpty($data[$i]['comprobante_id'])) {
                $stringVisualizarCompronbante = '<a  onclick="visualizarDocumento(' . $data[$i]['comprobante_id'] . ',' . $data[$i]['comprobante_movimiento_id'] . ')" title="Visualizar" style="cursor:pointer;color:-webkit-link;    text-decoration: underline;">' . $data[$i]['comprobante_serie_numero'] . '</a>';
            } elseif (!ObjectUtil::isEmpty($data[$i]['comprobante_id'])) {
                $stringVisualizarCompronbante = $data[$i]['comprobante_serie_numero'];
            }
              //::DESARROLLO JESUS::
            if($responseOpcion[0]['id'] == 227){
                if( $estadoId == 10){
                    $data[$i]['acciones'] = $stringAcciones . $stringAcciones2 ; 
     ...
     public function guardarEstadoObservacionDocumento(){
        $tipoEstado = $this->getParametro("tipoEstado");
        $descripcion =$this->getParametro("descripcion");
        $idDocumento =$this->getParametro("idDocumento");
        $usuarioId = $this->getUsuarioId();
         return ObservacionNegocio::create()->  cambiarEstadoDocumentoObservacion($idDocumento, $tipoEstado, $descripcion,   $usuarioId) ;
     }
    ...
    public function anular() {
        ...
        if ($idNegocio == DocumentoTipoNegocio::IN_PEDIDO ||
    ...
    public function imprimirQrPaquetesXPedido() {
        $documentoId = $this->getParametro("documentoId");
        return PedidoNegocio::create()->imprimirQrPaquetesXPedido($documentoId);
    }
    ...
    public function imprimirDocumentoGuiaRT() {
        $documentoId = $this->getParametro("id");
        return MovimientoNegocio::create()->imprimirDocumentoGuiaRT($documentoId);
    }

    public function obtenerDocumentoXId() {
        $documentoId = $this->getParametro("id");
        return PedidoNegocio::create()->obtenerDocumentoXId($documentoId);
    }

    public function obtenerXAgenciaIdXPersonaId() {
        $agenciaOrigenId = $this->getParametro("agenciaOrigenId");
        $agenciaDestinoId = $this->getParametro("agenciaDestinoId");
        $personaId = $this->getParametro("personaId");
        return PedidoNegocio::create()->obtenerXAgenciaIdXPersonaId($agenciaOrigenId, $agenciaDestinoId, $personaId);
    }
    public function imprimirDocumentoEntrega(){
        $documentoId = $this->getParametro("documentoId");
        $movimientoId = $this->getParametro("movimientoId");
        return PedidoNegocio::create()->GenerarConstanciaEntregaPdf($documentoId, $movimientoId);
    }
