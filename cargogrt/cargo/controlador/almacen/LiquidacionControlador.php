...
require_once __DIR__ . '/../../modeloNegocio/almacen/LiquidacionNegocio.php';
...
class LiquidacionControlador extends MovimientoControlador {

    public function obtenerConfiguracionesInicialesLiquidacion() {
        ...
        $data->acciones = MovimientoNegocio::create()->obtenerMovimientoTipoAccioneXOpcionIdXTipo($opcionId, $usuarioId);
    ...
    public function obtenerConfiguracionesIniciales() {
        ...
        return LiquidacionNegocio::create()->obtenerConfiguracionInicial($opcionId, $empresaId, $usuarioId, $documentoId, $movimientoTipoId);
    }
...
//        return MovimientoNegocio::create()->validarBienesFaltantes($opcionId, $usuarioId, $documentoTipoId, $camposDinamicos, $detalle, $documentoARelacionar, $valorCheck, $comentario,$checkIgv,$monedaId,$accionEnvio);
    ...
    public function obtenerDocumentosLiquidacionExcelXCriterios() {
        ...
        return LiquidacionNegocio::create()->obtenerDocumentosLiquidacionExcelXCriterios($opcionId, $criterios);
    }
    ...
    public function obtenerDocumentos() {
        ...
        $data = LiquidacionNegocio::create()->obtenerDocumentosLiquidacionXCriterios($opcionId, $criterios, $elemntosFiltrados, $columns, $order, $start);
...
//        $responseAcciones = MovimientoNegocio::create()->obtenerMovimientoTipoAcciones($opcionId);
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            for ($j = 0; $j < count($responseAcciones); $j++) {
                if ($estadoId == 2) { //ANULAR
                    $stringAcciones .= '';
                } elseif ($estadoId != 8 && $responseAcciones[$j]['id'] == 19) { //EDICION
                    $stringAcciones .= '';
    ...
    public function obtenerNotaVentaDocumentos() {
        // seccion de obtencion de variables
        $criterios = $this->getParametro("criterios");
        $elemntosFiltrados = $this->getParametro("length");
        $order = $this->getParametro("order");
        ...
        // seccion de consumir negocio
        $data = LiquidacionNegocio::create()->obtenerDocumentosNotaVentaXCriterios($criterios, $elemntosFiltrados, $columns, $order, $start);
        $elementosTotales = (count($data) > 0 ? $data[0]["total_filas"] : 0);
        return $this->obtenerRespuestaDataTable($data, $elementosTotales, $elementosTotales);
    }
    ...
    public function anular() {
        ...
        if ($idNegocio == DocumentoTipoNegocio::IN_PEDIDO || ($dataEmpresa[0]['efactura'] == 1 && (
    ...
    public function obtenerDocumentoRelacionVisualizar() {
        ...
        return LiquidacionNegocio::create()->visualizarDocumento($documentoId, $movimientoId);
    }
    ...
    public function obtenerDocumentoRelacionNotaVenta() {

        $documentoPorRelacionar = $this->getParametro("documentoPorRelacionar");
        $documentoRelacionados = $this->getParametro("documentoRelacionados");

        return LiquidacionNegocio::create()->obtenerDocumentoRelacionNotaVenta($documentoPorRelacionar, $documentoRelacionados);
    }
    ...
    public function enviarEdicion() {
        ...
        $distribucionObligatoria = $this->getParametro("distribucionObligatoria");
        $datosExtras = $this->getParametro("datosExtras");
        return MovimientoNegocio::create()->guardarXAccionEnvioEdicion($documentoId, $opcionId, $usuarioId, $documentoTipoId, $camposDinamicos, $detalle, $listaDetalleEliminar, $documentoARelacionar, $valorCheck, $comentario, $checkIgv, $monedaId, $accionEnvio, $tipoPago, $listaPagoProgramacion, $anticiposAAplicar, $periodoId, $percepcion, $detalleDistribucion, $contOperacionTipoId, $distribucionObligatoria, $afectoAImpuesto, $datosExtras);
    }
